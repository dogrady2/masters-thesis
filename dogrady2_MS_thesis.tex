...
For example, in Figure \ref{lf_his1} the linear heat rates calculated at hour 175 are assumed to be the conditioned linear heat rates for the following two power maneuvers, $t_{con}=150$. 

The weighted change in linear heat rate accounts for all parameters that effect \gls{PCI}.
The relative change from conditioned power determines if elevated stress are being applied on the claddings inner surface.
...
Due to the fact that gap width is a complex function of power history, the fuels linear heat rate and burnup are used to determine the likelihood the gap has closed.
...
An important parameter to consider is the effect prolonged load follow operation will have on the fuel's response.
...
Measured quantities including fuel centerline temperature, volume of gas released and pin diameter, are compared at \gls{BOL} and \gls{EOL} \cite{williamson_validating_2016}.
...
Regardless, BISON has been shown to capture complex thermo-mechanical behavior such as PCI failures in PWRs \cite{montgomery_advanced_2014,capps_pci_2017}.
...
In the remaining sections the BISON fuel performance and MPACT screening results for the first 24 of operation are presented.
The BISON fuel performance results are broken into four sections, the initial power ramp, and the first, second and third period of load-follow operation.
...
\end{figure}
\end{landscape}
\subsection{Load-follow Ramp 3}

%#  673  python ~/python_files/lf_screen_v0.py lfboc 39 1
%#  674  python ~/python_files/lf_screen_v0.py lfboc 39 2
%#  675  python ~/python_files/lf_screen_v0.py lfboc 98 2
%#  676  python ~/python_files/lf_screen_v0.py lfboc 98 6
%#  681  python ~/python_files/lf_screen_v0.py lf390h 22 6
%#  682  python ~/python_files/lf_screen_v0.py lf390h 47 6
%#  683  python ~/python_files/lf_screen_v0.py lf390h 74 6
